...
pub fn generate_client(idl: &SpelIdl) -> Result<String, String> {
    ...
    writeln!(out, "use wallet::{{AccountIdentity, WalletCore}};").unwrap();
    writeln!(out).unwrap();
        ...
        .filter(|ix| crate::instruction_supports_public_submission(ix))
    {
        ...
        // Fail explicitly when an IDL-declared signer is not owned. WalletCore's
        // Public identity signs opportunistically, so this keeps the IDL contract strict.
        let signers: Vec<_> = ix.accounts.iter().filter(|a| a.signer).collect();
        for s in &signers {
            let name = rust_ident(&s.name);
            if s.rest {
                writeln!(out, "        for signer_id in &accounts.{name} {{").unwrap();
                writeln!(out, "            if self.wallet.get_account_public_signing_key(*signer_id).is_none() {{").unwrap();
                writeln!(out, "                return Err(format!(\"signing key not found for {{}}\", signer_id));").unwrap();
                writeln!(out, "            }}").unwrap();
                writeln!(out, "        }}").unwrap();
            } else {
                writeln!(out, "        if self.wallet.get_account_public_signing_key(accounts.{name}).is_none() {{").unwrap();
                writeln!(out, "            return Err(format!(\"signing key not found for {{}}\", accounts.{name}));").unwrap();
                writeln!(out, "        }}").unwrap();
            }
        }
        ...
        for acc in &ix.accounts {
            ...
            if acc.rest {
                writeln!(out, "        for account_id in accounts.{name} {{").unwrap();
                writeln!(
                    out,
                ...
                writeln!(out, "        account_identities.push(AccountIdentity::{identity}(accounts.{name}));").unwrap();
            }
        }
        writeln!(
            out,
        ...
        .unwrap();
        writeln!(out, "        let response = self.wallet.send_pub_tx(account_identities, instruction_data, self.program_id).await").unwrap();
        writeln!(
            out,
            "            .map_err(|e| format!(\"submit: {{}}\", e))?;"
        )
        .unwrap();
        writeln!(out, "        Ok(response.to_string())").unwrap();
        writeln!(out, "    }}").unwrap();
    }
    ...
    for helper in &pda_helpers {
        ...
        .unwrap();
        writeln!(out, "        T::try_from_slice(account.data.as_ref())").unwrap();
        writeln!(
            out,
