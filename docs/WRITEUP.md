...
The privacy property documented in this writeup is scoped to the `claim` instruction. The program exposes two claim paths and they have different observer-visibility properties.

**`claim` (Risc0 receipt path, `methods/guest/src/bin/distributionx.rs:379-466`).** The instruction args are `airdrop_id`, `nullifier`, `receipt_bytes`, and `now_unix`. Verification consumes a Risc0 Groth16 receipt and reads the journal. The public journal contains only `airdrop_id`, `merkle_root`, `bucket_id`, `nullifier`, and `claim_destination_commitment` (`crates/distributionx-circuit/src/journal.rs:3-10`). The eligible address, row salt, claim signature, and Merkle path are private witness inputs to the zkVM and never appear in the journal or in the instruction data. This is the path the privacy property above refers to.

**`claim_private` (in-program verifier, `methods/guest/src/bin/distributionx.rs:470-553`).** This instruction runs the witness verification inside the program rather than inside the zkVM, so the witness fields have to be passed as instruction args. The IDL at `idl/distributionx.json:204-216` lists them: `claimant_address`, `salt`, `claim_sig`, `merkle_siblings`, `merkle_path_is_right`. The generated FFI submits via `NSSATransaction::Public(tx)` at every send site (`src/generated/distributionx_ffi.rs:249, 315, 388, 471, 535`), so those arguments are part of the public transaction transcript and an observer with RPC `getTransaction` access can recover them. `claim_private` is kept in the program as a fallback verifier and does not provide observer privacy for the witness.

**A note on naming.** The `claim_private` identifier predates this audit and is misleading in its current form. The instruction does not provide observer privacy for the witness; it runs the in-program verifier as described above. A clearer identifier would be something like `claim_inline` (because the verifier runs inline in the LEZ program rather than inside the zkVM). The rename is deferred because it touches the LEZ program, all three IDL mirrors (`idl/`, `crates/distributionx-program/idl/`, `crates/distributionx-bindings/generated/`), the generated client and FFI in `src/generated/`, scripts, tests, and several doc sections (about 17 files in total). The documentation above describes what the instruction actually does; the rename is tracked as a follow-up so this change can focus on scoping the privacy claim correctly and flipping the demo default.
...
**Public exposure.** Salts are not in the Risc0 `claim` path's journal or instruction data. Under the `claim_private` path the salt is part of the instruction args (see Privacy Model above); the default demo flow does not use that path, so the chain transcript for the demo does not carry salts.

## LEZ Account Model

The IDL in `crates/distributionx-program/idl/distributionx.json` defines `Airdrop` and `NullifierRecord` PDAs, plus `init_airdrop`, `fund`, `claim`, `claim_private`, and `close`. The default demo flow submits the `claim` instruction with a Risc0 receipt: the program verifies the receipt, checks the journal against airdrop state, debits the vault, records the nullifier atomically, and emits the local submit receipt. The local submit adapter then settles the configured custom token transfer to the destination commitment account.

The `claim_private` instruction remains in the program as a fallback verifier that runs the witness check inside the program instead of inside the zkVM. It is documented for completeness; it is not used by the default demo flow and does not provide observer privacy for the witness.
...
That flow preserves the shielded destination packet, generates a real Risc0 receipt with `RISC0_DEV_MODE=0`, submits the LEZ `claim` transaction (the demo default; the `should_use_private_claim` gate in `scripts/local-submit.sh` routes to the receipt-based `claim` instruction unless `DISTRIBUTIONX_USE_CLAIM_PRIVATE` is set), rejects a duplicate local claim, and closes the airdrop on-chain.
